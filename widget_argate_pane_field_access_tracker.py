...
class WIDGET_ARGATE_PANE_FIELD_ACCESS_TRACKER(QWidget):
    ...
    def __init__(self, app_data):
        ...
        self.listbox_Tracker.setModel(self.appData.model_FieldAccessTrackerView)
    ...
    @pyqtSlot(str)
    def msgprc_OnTrackFieldAccess(self, str):

        text = str

        #self.list_TrackerList.ScrollHint = QAbstractItemView.PositionAtBottom

        self.appData.model_FieldAccessTracker.theList.append(
            QDateTime.currentDateTime().toString("dd.MM.yyyy hh:mm:ss.zzz") + " - " + text)
        #(?)self.list_TrackerList.scrollTo(QModelIndex(len(self.appData.model_FieldAccessTracker.theList)))
        #ind = self.appData.model_FieldAccessTracker.index()
        #kkk = self.appData.model_FieldAccessTracker.rowsInList()

        if len( self.appData.model_FieldAccessTracker.theList ) > 100:
            self.appData.model_FieldAccessTracker.theList.pop(0)


        #self.list_TrackerList.scrollTo(self.appData.model_FieldAccessTracker.index(self.appData.model_FieldAccessTracker.rowsInList()))


        self.appData.model_FieldAccessTracker.layoutChanged.emit()
        self.list_TrackerList.scrollToBottom()

        #self.list_TrackerList.scrollTo(self.list_TrackerList.currentIndex())

        #index = self.list_TrackerList.currentIndex()
        #row = index.row()
    ...
    def msgprc_OnResumeButtonClicked(self):
        ...
        n = self.appData.model_FieldAccessTrackerView.capacity

        if self.appData.model_FieldAccessTrackerBack.count() > n:
            self.appData.model_FieldAccessTrackerView.theList[:] = self.appData.model_FieldAccessTrackerBack.theList[-n:]
        else:
            self.appData.model_FieldAccessTrackerView.theList[:] = self.appData.model_FieldAccessTrackerBack.theList[:]
    ...
    def msgprc_OnExportButtonClicked(self):
        ...
        if fname != "":
            ...
            self.exporter = UDT_TRACKER_EXPORTER(self.appData.model_FieldAccessTrackerBack.theList, fname)
